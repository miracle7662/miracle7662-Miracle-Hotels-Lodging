...
//   // Normalize API district item into a consistent shape
//   const normalizeDistrict = (d) => ({
//     districtid: d?.distrcitid ?? null,
...
//     try {
//       if (editingDistrict) {
//         const response = await axios.put(`http://localhost:3001/api/districts/${editingDistrict.districtid}`, formData);
...
//           setDistricts(prev => prev.map(d => d.districtid === normalizedDistrict.districtid ? normalizedDistrict : d));
//         } else {
//           await loadDistricts(); // Fallback to reload if no district in response
//         }
//       } else {
...
//     setFormData({
//       distrcitname: district.district_name,
//       ditcrictcode: district.ditcrictcode,
...
//     if (districtToDelete) {
//       try {
//         console.log('Deleting district with ID:', districtToDelete.districtid);
//         await axios.delete(`http://localhost:3001/api/districts/${districtToDelete.districtid}`);
//         await loadDistricts();
...
//     ((district?.ditcrictcode || '').toLowerCase().includes(normalizedSearch)) ||
//     ((district?.description || '').toLowerCase().includes(normalizedSearch)) ||
//     ((district?.state_name || '').toLowerCase().includes(normalizedSearch)) ||
//     ((district?.country_name || '').toLowerCase().includes(normalizedSearch))
//   );
...
//                             {district.created_date ? new Date(district.created_date).toLocaleDateString() : '-'}
//                           </small>
//                         </td>
//                         <td>
...
const DistrictMaster = () => {
  ...
  const confirmDelete = async () => {
    if (districtToDelete) {
      try {
        console.log(`Deleting district with ID: ${districtToDelete.districtid}`);
        const response = await axios.delete(`http://localhost:3001/api/districts/${districtToDelete.districtid}`);
        console.log('Delete response:', response.data);
        if (response.data.message === 'District deleted successfully!') {
          setDistricts(prev => prev.filter(d => d.districtid !== districtToDelete.districtid));
          await loadDistricts(); // Refresh to ensure consistency
          setError(''); // Clear any previous error
        } else {
          throw new Error('Delete operation failed unexpectedly');
        }
      } catch (err) {
        setError(err.response?.data?.error || 'Failed to delete district');
        console.error('Error deleting district:', err.response ? err.response.data : err.message);
      } finally {
        setShowDeleteModal(false);
        setDistrictToDelete(null);
      }
